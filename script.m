function script()
    ...
    taille_population = 150;       % Tamanho da população (150)
    n_generations = 1200;          % Número de gerações (1200)
    taux_mutation = 0.02;          % Taxa de mutação (2%)
    ...
    for generation = 1:n_generations
        ...
        % Exibir o melhor resultado a cada 100 gerações
...
function fitness = evaluer_population(population)
    fitness = zeros(size(population, 1), 1); % Inicializa o vetor de fitness
    for idx = 1:size(population, 1)
        [x, y] = decode(population(idx, :)); % Decodifica para obter x e y
        fitness(idx) = -eggholder(x, y);       % Calcula o fitness
    end
end
...
function [x, y] = decode(individu)
    ...
    % Converte os bits em um número decimal e escala para [0, 1000]
    ...
    for i = 1:size(mutes, 1)
        for j = 1:size(mutes, 2)
            if rand() < taux_mutation
                mutes(i, j) = 1 - mutes(i, j); % Inverte o bit
            end
        end
    end
end
